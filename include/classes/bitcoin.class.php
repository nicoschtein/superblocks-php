...
require_once(LIB_DIR . "/xmlrpc.inc.php");
require_once(LIB_DIR . "/jsonrpc.inc.php");
...
class BitcoinClient extends jsonrpc_client {
  ...
  /**
   * Returns the hash for a block number.
   *
   * @return integer Block hash for a block number
   * @throws BitcoinClientException
   */
  public function getblockhash($number) {
    if (!$number || empty($number))
      throw new BitcoinClientException("getblockhash requires a block number");
    return $this->query("getblockhash", $number);
  }
